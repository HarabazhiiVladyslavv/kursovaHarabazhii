...
if (isset($_POST['orders'])){
    ...
    if ($name && $phone && is_numeric($sticksNum) && is_numeric($educSticksNum) && is_numeric($peopleNum) && $adressHouse && $cityName && $streetName && $buildNum) { // Якщо всі поля заповнено
        $cnct = new mysqli($servername, $username, $password,'sushiRolls'); // Підключення до бази даних

        if ($cnct->connect_error) {
            die("Помилка підключення до бази даних: " . $cnct->connect_error);
        }
        $date = date('Y-m-d'); // Отримання поточної дати
  $sql = "INSERT INTO `orders` (`name`, `phone`, `sticksNum`, `educSticksNum`, `peopleNum`, `adressHouse`, `cityName`, `streetName`, `buildNum`, `floorNumb`, `entranceNumb`, `apartmentNumb`, `userMessage`, `jsonOrders`, `date`) VALUES ('$name', '$phone', '$sticksNum', '$educSticksNum', '$peopleNum', '$adressHouse', '$cityName', '$streetName', '$buildNum', '$entranceNumb', '$floorNumb', '$apartmentNumb', '$userMessage', '$jsonOrders', '$date');";
...
$cnct->close(); // Закриваємо з'єднання з базою даних
header('location: /thanks_for_order.html');
exit();
} else {
$error = 'Помилка при виконанні запиту: ' . $cnct->error;
} $cnct->close(); // Закриваємо з'єднання з базою даних
} else {
